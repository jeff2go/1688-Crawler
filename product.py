...
class Product():
    ...
    # 图片
    def __extract_imgs(self, tree):
        img_elements = tree.xpath('//div[@id="dt-tab"]//a[@class="box-img"]/img')
        return list(map(lambda img_element: img_element.attrib['src'].replace('.60x60', ''), img_elements))
    ...
    # 详情
    def __extract_detail(self, tree):
        detail_request_url = tree.xpath('//div[@id="desc-lazyload-container"]')[0].attrib['data-tfs-url']
        content = self.__fetch_content(detail_request_url)
    ...
    def go(self, url):
        ...
        product['imgs'] = self.__extract_imgs(tree)
        product['attributes'] = self.__extract_attributes(tree)
        product['detail'] = self.__extract_detail(tree)
