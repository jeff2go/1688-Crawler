...
PRODUCT_IMG_REPLACE_TARGET = '.60x60'
headers = {
    "Referer": "https://ywlingpan.1688.com/page/offerlist.htm?spm=a2615.7691456.0.0.4439bf5bFsNPRz",
...
# 标题
product['title'] = tree.xpath('//h1[@class="d-title"]')[0].text

# 图片
img_elements = tree.xpath('//div[@id="dt-tab"]//a[@class="box-img"]/img')
product['imgs'] = list(map(lambda img_element: img_element.attrib['src'].replace(PRODUCT_IMG_REPLACE_TARGET, ''), img_elements))

# 参数
attribute_feature_elements = tree.xpath('//td[@class="de-feature"]')
attribute_value_elements = tree.xpath('//td[@class="de-value"]')
product['attributes'] = list(map(lambda feature, value: {'feature': feature.text, 'value': value.text}, attribute_feature_elements, attribute_value_elements))

# 详情
desc_request_url = tree.xpath('//div[@id="desc-lazyload-container"]')[0].attrib['data-tfs-url']
headers['Referer'] = PRODUCT_URL
r = s.get(desc_request_url, headers=headers)
product['desc'] = r.text[30:-3].replace('\\', '')
